...
import { musicStore, type Song, setCurrentSong, togglePlay } from '@/domains/music/music.store';
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex items-center justify-center">
                                    {isCurrent && isPlaying ? (
                                        <div className="flex items-end gap-0.5 h-3">
                                            <motion.div animate={{ height: [4, 12, 6, 12] }} transition={{ repeat: Infinity, duration: 0.6 }} className="w-0.5 bg-purple-500" />
                                            <motion.div animate={{ height: [12, 4, 12, 8] }} transition={{ repeat: Infinity, duration: 0.7 }} className="w-0.5 bg-purple-500" />
                                            <motion.div animate={{ height: [8, 12, 4, 12] }} transition={{ repeat: Infinity, duration: 0.5 }} className="w-0.5 bg-purple-500" />
                                        </div>
                                    ) : (
                                        <>
                                            <span className={`text-sm group-hover:hidden ${isCurrent ? 'text-pink-400' : 'text-gray-400'}`}>
                                                {index + 1}
                                            </span>
                                            <Play className={`w-3 h-3 hidden group-hover:block ${isCurrent ? 'text-pink-400 fill-pink-400' : 'text-white fill-white'}`} />
                                        </>
                                    )}
                                </div>
                                ...
                                <div className="flex items-center gap-4 min-w-0">
                                    <img src={song.albumArt} alt="" className="w-10 h-10 rounded shadow-md" />
                                    <div className="min-w-0">
                                        <div className={`font-medium truncate ${isCurrent ? 'text-pink-400' : 'text-white'}`}>
                                            {song.title}
                                        </div>
