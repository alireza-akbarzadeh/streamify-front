...
export function SidebarItem({ item, onOpenCreate }: SidebarItemProps) {
    ...
    const handlePin = () => {
        togglePin(item.id);
        toast.success(item.isPinned ? "Unpinned from Library" : "Pinned to Library", {
    ...
    const handleShare = (type: 'link' | 'embed') => {
        const text = type === 'link' ? "Link copied to clipboard" : "Embed code copied";
        ...
        <ContextMenu>
            ...
            <ContextMenuContent className="w-64 bg-[#282828] border-[#3e3e3e] text-[#e0e0e0] shadow-xl">
                ...
                <ContextMenuItem
                    ...
                    className="gap-3 py-2.5 focus:bg-white/10 focus:text-white cursor-pointer"
                >
                    <Pin className={`w-4 h-4 ${item.isPinned ? 'text-purple-500 fill-purple-500' : 'text-gray-400'}`} />
                ...
                <ContextMenuSub>
                    ...
                    <ContextMenuSubContent className="bg-[#282828] border-[#3e3e3e] text-white">
                        <ContextMenuItem onClick={() => handleShare('link')}>Copy Playlist Link</ContextMenuItem>
                        <ContextMenuItem onClick={() => handleShare('embed')}>Embed Playlist</ContextMenuItem>
                    </ContextMenuSubContent>
                </ContextMenuSub>
            </ContextMenuContent>
        </ContextMenu>
