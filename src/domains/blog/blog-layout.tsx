...
export function BlogLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex flex-col lg:flex-row min-h-screen bg-[#050505] text-white selection:bg-purple-500/30">
            ...
            {isSmallScreen && (
                <header className="sticky top-0 w-full h-16 bg-[#080808]/80 backdrop-blur-2xl border-b border-white/5 z-60 px-4 flex items-center justify-between">
                    <Link to="/" className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-black uppercase tracking-tighter">{MSG.APP_NAME}</span>
                    </Link>
                ...
                <div className="flex-1 overflow-y-auto px-4 mt-20 lg:mt-0 custom-scrollbar">
                    {!isSmallScreen && (
                        ...
                        >
                            ...
                            <kbd className="text-[10px] font-bold bg-black px-1.5 py-0.5 rounded border border-white/10">⌘K</kbd>
                        </button>
                    )}
                            ...
                            {CATEGORIES.map((cat) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <cat.icon size={18} className={`relative z-10 ${isActive ? 'text-purple-400' : ''}`} />
                    ...
                    <div className="mb-8">
                        ...
                        <div className="space-y-1">
                            <LibraryButton icon={Heart} label="Liked" count={likes.length} color="text-pink-500" active={activeCategory === 'likes'} onClick={() => { actions.setActiveCategory('likes'); if (isSmallScreen) setIsMenuOpen(false); }} />
                            <LibraryButton icon={Sparkles} label="Saved" count={bookmarks.length} color="text-yellow-400" active={activeCategory === 'bookmarks'} onClick={() => { actions.setActiveCategory('bookmarks'); if (isSmallScreen) setIsMenuOpen(false); }} />
                        </div>
                    </div>
                ...
                <div className="p-4 border-t border-white/5 mt-auto">
                    ...
                    >
                        <div className="flex items-center gap-3 p-2 rounded-2xl hover:bg-white/5 transition-colors cursor-pointer group outline-none">
                            <div className="relative">
                                <img alt='user' src="https://i.pravatar.cc/100?u=me" className="w-10 h-10 rounded-xl object-cover ring-2 ring-white/5 group-hover:ring-purple-500/50 transition-all" />
                            ...
                            <ChevronRight size={16} className="text-neutral-600 group-hover:text-white transition-all group-data-[state=open]:rotate-180" />
                        </div>
                    </Link>
                </div>
            </aside>
                    ...
                    >
                        ...
                        >
                            <div className="w-6 h-6 rounded-full overflow-hidden border border-white/20">
                                <img src="https://i.pravatar.cc/100?u=alireza" className="w-full h-full object-cover" alt="p" />
                            </div>
                        </Link>
                    </motion.nav>
                )}
            ...
            <main className={`flex-1 ${isSmallScreen ? 'min-h-[calc(100vh-64px)]' : 'h-screen overflow-y-auto'}`}>
                {children}
