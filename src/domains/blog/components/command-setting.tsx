        ...
        <CommandDialog open={open} onOpenChange={setOpen} >
            ...
            <CommandList className="custom-scrollbar">
                ...
                <CommandGroup heading="Browse Categories">
                    {CATEGORIES.map((cat) => (
                        ...
                        >
                            ...
                            <span>{cat.label}</span>
                            <CommandShortcut className="text-[10px]">G + {cat.label[0]}</CommandShortcut>
                        </CommandItem>
                    ))}
                </CommandGroup>
                {/* --- 3. QUICK ACTIONS --- */}
                <CommandGroup heading="Quick Actions">
                    <CommandItem onSelect={() => { actions.setActiveCategory('bookmarks'); setOpen(false); }} className="cursor-pointer">
                        <Sparkles className="mr-2 h-4 w-4 text-yellow-400" /> View Bookmarks
                    </CommandItem>
                    <CommandItem onSelect={() => { actions.setActiveCategory('likes'); setOpen(false); }} className="cursor-pointer">
                ...
                <CommandGroup heading="Settings & Account">
                    <CommandItem onSelect={() => { /* Navigate to profile */ setOpen(false); }} className="cursor-pointer">
                        <User className="mr-2 h-4 w-4" /> Profile Settings
                    </CommandItem>
                    <CommandItem onSelect={() => { /* Navigate to billing */ setOpen(false); }} className="cursor-pointer">
                        <CreditCard className="mr-2 h-4 w-4" /> Subscription & Billing
                    </CommandItem>
                    <CommandItem onSelect={() => { /* Log out logic */ setOpen(false); }} className="cursor-pointer text-red-500">
