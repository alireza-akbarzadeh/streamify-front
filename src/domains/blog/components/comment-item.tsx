...
import { Heart, MoreHorizontal, Reply, Send, ShieldCheck, Sparkles, User } from 'lucide-react';
...
export function CommentItem({ comment, isLatest }: CommentProps) {
    ...
    const isInlineReplying = useStore(commentStore, (state) => state.replyingToId === comment.id);
        ...
        >
            ...
            <div className="relative shrink-0">
                <motion.img
                    ...
                    className={`w-12 h-12 md:w-16 md:h-16 rounded-[1.5rem] object-cover border-2 transition-all duration-500 ${isLatest ? 'border-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.5)]' :
                        isAdmin ? 'border-purple-500/40' : 'border-white/5'
                        }`}
                />
                {isAdmin && (
            ...
            <div className="flex-1 flex flex-col gap-3">
                {/* Main Card */}
                <div className={`relative rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 transition-all duration-700
                    ${isLatest
                ...
                >
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            <span className={`font-black text-lg italic tracking-tight ${isAdmin || isLatest ? 'text-purple-400' : 'text-white'}`}>
                                {comment.author.name}
                            </span>
                            ...
                            <span className="text-[10px] font-bold text-neutral-600 uppercase tracking-widest">{comment.timestamp}</span>
                        </div>
                        <button type='button' className="text-neutral-600 hover:text-white transition-colors">
                            <MoreHorizontal size={18} />
                        </button>
                    </div>

                    <p className={`text-base md:text-lg leading-relaxed mb-6 ${isAdmin || isLatest ? 'text-neutral-200' : 'text-neutral-400'}`}>
                        {comment.content}
                    </p>

                    <div className="flex items-center gap-6 border-t border-white/5 pt-6">
                        <button type='button' className="group flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-neutral-500 hover:text-pink-500 transition-all">
                            <Heart size={14} className="group-hover:fill-current" /> {comment.likes}
                        </button>
                        <button
                            type='button'
                        ...
                        >
                            <Reply size={14} className={isInlineReplying ? 'rotate-180 transition-transform' : ''} />
