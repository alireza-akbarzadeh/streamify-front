...
import { ArrowLeft, Bookmark, CheckCircle2, Clock, Edit3, Heart, History, Zap } from 'lucide-react';
...
export function UserProfilePage() {
    ...
    const displayArticles = useMemo(() => {
        switch (activeTab) {
            case 'saved': return MOCK_ARTICLES.filter(a => bookmarks.includes(a.id));
            case 'liked': return MOCK_ARTICLES.filter(a => likes.includes(a.id));
            case 'history': return MOCK_ARTICLES.filter(a => finished.includes(a.id));
            default: return [];
        }
    }, [activeTab, bookmarks, likes, finished]);

    const stats = [
        { label: 'Saved', value: bookmarks.length, icon: Bookmark, color: 'text-purple-400' },
        { label: 'Liked', value: likes.length, icon: Heart, color: 'text-pink-500' },
        { label: 'Read', value: finished.length, icon: CheckCircle2, color: 'text-emerald-400' },
                    ...
                    <div className="flex flex-col md:flex-row items-center md:items-end gap-6 text-center md:text-left">
                        ...
                        >
                            ...
                            <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-[2rem] bg-[#080808] border-2 border-white/10 overflow-hidden">
                                <img src="https://i.pravatar.cc/150?u=alireza" className="w-full h-full object-cover" alt="User" />
                            </div>
                        </motion.div>

                        <div className="flex-1 space-y-2">
                            <div className="flex flex-col md:flex-row items-center gap-3">
                                <h1 className="text-4xl md:text-6xl font-black italic tracking-tighter uppercase">Alireza</h1>
                                <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[10px] font-black uppercase tracking-widest text-purple-400">Editorial Member</span>
                            </div>
                            <p className="text-neutral-500 text-sm font-medium italic flex items-center justify-center md:justify-start gap-2">
                                <Clock size={14} className="text-purple-500" /> Active since Jan 2024
                            </p>
                        </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                    {stats.map((stat, i) => (
                        ...
                        >
                            ...
                            <div className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-600">{stat.label}</div>
                        </motion.div>
                    ))}
                </div>
            </div>
                ...
                <div className="flex items-center gap-6 md:gap-10 border-b border-white/5 mb-8 md:mb-10 overflow-x-auto no-scrollbar scroll-smooth">
                    {tabs.map((tab) => (
                        <Button
                            ...
                            className={`pb-4 text-[10px] font-black uppercase tracking-[0.2em] transition-all relative whitespace-nowrap ${activeTab === tab.id ? 'text-white' : 'text-neutral-600 hover:text-neutral-400'
                                }`}
                        >
                            {tab.label}
                            {activeTab === tab.id && (
                    ...
                    >
                        {displayArticles.length > 0 ? (
                            ...
                            <div className="py-20 md:py-32 text-center border border-dashed border-white/10 rounded-[2rem] md:rounded-[3rem] bg-white/[0.01]">
                                ...
                                <h3 className="text-xl font-bold text-neutral-500 uppercase tracking-tighter">Your shelf is empty</h3>
                                <p className="text-neutral-700 text-xs md:text-sm italic mt-1 px-6">
                                    {activeTab === 'saved' ? "Found something interesting? Bookmark it to see it here." :
                                        activeTab === 'liked' ? "Spread some love to articles to populate this list." :
