...
const RATING_CONFIG: Record<Rating, { emoji: string; color: string; bg: string }> = {
    "Mind Blown": { emoji: "🤯", color: "text-yellow-400", bg: "bg-yellow-400/10 border-yellow-400/20" },
    "Lit": { emoji: "🔥", color: "text-orange-500", bg: "bg-orange-500/10 border-orange-500/20" },
    "Love": { emoji: "❤️", color: "text-pink-500", bg: "bg-pink-500/10 border-pink-500/20" },
    "Interesting": { emoji: "🧐", color: "text-blue-400", bg: "bg-blue-400/10 border-blue-400/20" },
    "Electric": { emoji: "⚡", color: "text-purple-400", bg: "bg-purple-400/10 border-purple-500/20" },
};
...
export function ArticleCard({ article, index }: ArticleCardProps) {
    ...
    const ratingStyle = RATING_CONFIG[article.rating as Rating] || RATING_CONFIG["Interesting"];

    const categoryColor = article.category.toLowerCase() === 'music'
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center">
                        <div className="flex -space-x-2 mr-3">
                            {[1, 2, 3].map((i) => (
                                <img key={i} className="w-6 h-6 rounded-full border-2 border-[#050505] object-cover" src={`https://i.pravatar.cc/100?img=${i + 22}`} alt="reader" />
                            ))}
                        </div>
                        <span className="text-[9px] text-neutral-500 font-bold uppercase tracking-widest">+2.4k Reading</span>
                    </div>

                    {/* Integrated Rating Pill in Body */}
                    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border transition-colors duration-300 ${ratingStyle.bg}`}>
                        <span className="text-[10px] leading-none">{ratingStyle.emoji}</span>
                        <span className={`text-[9px] font-black uppercase tracking-widest ${ratingStyle.color}`}>
                            {article.rating}
                        </span>
                    </div>
                </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        >
                            <div className="relative">
                                ...
                                {isLiked && (
                                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-1 -right-1 text-pink-400">
                                        <Sparkles size={10} />
                                    </motion.div>
                                )}
                            </div>
