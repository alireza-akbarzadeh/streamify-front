...
export const userColumns: ColumnDef<UserAccount>[] = [
            ...
            return (
                <div className="flex items-center gap-3 py-1">
                    <div className="relative shrink-0">
                        <img src={avatar} alt={name} className="h-10 w-10 rounded-xl bg-muted border border-border/50 object-cover" />
                        {role !== "User" && (
                            <div className={cn(
                    ...
                    <div className="flex flex-col min-w-0 max-w-[180px]">
                        <div className="flex items-center gap-1.5">
                            ...
                            {emailVerified && <CheckCircle2 className="h-3 w-3 text-emerald-500 shrink-0" />}
                        </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="flex -space-x-2">
                        {mfaMethods?.map((method) => (
                            <div key={method} className="h-7 w-7 rounded-full bg-background border-2 border-muted flex items-center justify-center shadow-sm">
                                {method === 'authenticator' ? <Fingerprint className="h-3.5 w-3.5 text-primary" /> : <Smartphone className="h-3.5 w-3.5 text-blue-500" />}
                            </div>
                        ))}
                        {!twoFactorEnabled && (
    ...
    // 4. USAGE & METRICS
    {
        ...
        cell: ({ row }) => {
            const { usage, storageUsedMB, streamingHours, profileComplete } = row.original
            return (
                <div className="flex flex-col gap-1.5 w-[140px]">
                    <div className="flex justify-between items-end">
                        <div className="flex flex-col">
                            <span className="text-[10px] font-black uppercase text-muted-foreground leading-none">Usage</span>
                            <span className="text-[11px] font-bold">{usage}%</span>
                        </div>
                        <div className="flex flex-col items-end">
                            <span className="text-[8px] font-bold text-muted-foreground uppercase">Setup</span>
                            <span className="text-[10px] font-bold text-primary">{profileComplete}%</span>
                        </div>
                    </div>
                    <div className="h-1.5 w-full bg-muted/30 rounded-full overflow-hidden">
                        <div className={cn("h-full bg-primary")} style={{ width: `${profileComplete}%` }} />
                    </div>
                    <div className="flex items-center justify-between text-[9px] text-muted-foreground font-medium">
                        <span className="flex items-center gap-0.5"><HardDrive className="h-2.5 w-2.5" /> {storageUsedMB}MB</span>
                        <span className="flex items-center gap-0.5"><Clock className="h-2.5 w-2.5" /> {streamingHours}h</span>
                    </div>
                </div>
            )
        }
        ...
        cell: ({ row }) => {
            ...
            return (
                <div className="flex flex-col">
                    ...
                    <span className="text-[9px] text-muted-foreground uppercase font-medium">Original Member</span>
                </div>
            )
        }
    },
            ...
            return (
                <div className="flex flex-col gap-1 min-w-[120px]">
                    ...
                    <div className="flex flex-col text-[10px] font-medium text-muted-foreground leading-tight">
                        ...
                        {subscriptionEnd && <span className="text-[9px] opacity-70">Ends: {new Date(subscriptionEnd).toLocaleDateString()}</span>}
                    </div>
                </div>
            )
        }
                ...
                <div className="flex flex-col gap-1.5">
                    ...
                    <div className="flex flex-wrap gap-1 max-w-[120px]">
                        {tags?.slice(0, 2).map(tag => (
                            <span key={tag} className="text-[8px] px-1 bg-muted rounded font-bold text-muted-foreground">#{tag}</span>
                        ))}
                    </div>
                </div>
            )
