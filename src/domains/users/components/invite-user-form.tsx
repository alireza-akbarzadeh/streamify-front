    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 pb-6">
            <div className="space-y-4">
                ...
                {/* Role Selection */}
                <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-muted-foreground ml-1">Access Level</Label>
                    <div className="grid grid-cols-3 gap-2">
                        {['User', 'Moderator', 'Admin'].map((role) => (
                            <label key={role} className="relative cursor-pointer group">
                                <input type="radio" name="role" value={role} className="peer sr-only" defaultChecked={role === 'User'} />
