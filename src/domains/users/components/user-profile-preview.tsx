import { BadgeCheck, CreditCard, Edit3, Mail, Phone, Save, Shield, ShieldAlert, X, Zap } from "lucide-react";
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-2">
                {[
                    { label: "Usage", value: `${user.usage}%`, icon: Zap, color: "text-amber-500" },
                    { label: "Storage", value: `${user.storageUsedMB}MB`, icon: BadgeCheck, color: "text-primary" },
                    { label: "Credits", value: user.credits, icon: Zap, color: "text-emerald-500" },
                ].map((stat) => (
                    <div key={stat.label} className="flex flex-col items-center p-3 rounded-2xl bg-muted/30 border border-border/40 transition-colors hover:bg-muted/50">
                        <stat.icon className={cn("h-3.5 w-3.5 mb-1", stat.color)} />
                        <span className="text-[11px] font-bold">{stat.value}</span>
                        <span className="text-[8px] uppercase text-muted-foreground font-black tracking-tighter">{stat.label}</span>
                    </div>
                ))}
            </div>
            ...
            <div className="flex items-center justify-between px-1">
                <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Core Identity</span>
                <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setIsEditing(!isEditing)}
                    className={cn("h-7 gap-1.5 text-[10px] font-bold rounded-lg", isEditing ? "text-destructive hover:bg-destructive/10" : "text-primary hover:bg-primary/10")}
                >
                    {isEditing ? <><X className="h-3 w-3" /> Cancel</> : <><Edit3 className="h-3 w-3" /> Edit Profile</>}
                </Button>
            </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-xl border border-border/50 bg-background/50 p-3">
                        ...
                        {isEditing ? (
                            ...
                            <span className="text-xs font-bold text-primary">{user.plan}</span>
                        )}
                    </div>
                    <div className="rounded-xl border border-border/50 bg-background/50 p-3">
                        <span className="text-[9px] font-bold text-muted-foreground uppercase flex items-center gap-1.5 mb-1">
...
function ProfileField({ label, value, isEditing, icon: Icon, verified }: ProfileFieldProps) {
    return (
        <div className="flex flex-col gap-1">
            <span className="text-[9px] font-black text-muted-foreground/70 uppercase tracking-tight">{label}</span>
            <div className="flex items-center gap-3">
                ...
                {isEditing ? (
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {verified && <BadgeCheck className="h-3.5 w-3.5 text-emerald-500" />}
                        {verified === false && <ShieldAlert className="h-3.5 w-3.5 text-amber-500" />}
                    </div>
                )}
            </div>
        </div>
