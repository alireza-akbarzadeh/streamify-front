...
export function UpdateStaffRole({ selectedId, setSelectedId, activeStaff, handlePermissionToggle, setStaff }: UpdateStaffRoleProps) {
    const [searchQuery, setSearchQuery] = useState("");

    // Filtered routes based on search
    const filteredRoutes = useMemo(() => {
        return SYSTEM_ROUTES.filter(r =>
    ...
    // Calculate stats
    const writeCount = Object.values(activeStaff?.activePerms || {}).filter(v => v === 'write').length;
    const readCount = Object.values(activeStaff?.activePerms || {}).filter(v => v !== null).length;

    const resetPermissions = () => {
        setStaff(prev => prev.map(s => s.id === selectedId ? { ...s, activePerms: {} } : s));
        toast.error("All manual overrides cleared");
    };

    return (
        <Sheet open={!!selectedId} onOpenChange={(open) => !open && setSelectedId(null)}>
            <SheetContent side="right" className="w-[550px] sm:w-[650px] bg-[#020617] border-l border-white/10 p-0 text-slate-200 shadow-2xl flex flex-col outline-none">
                    ...
                    <SheetHeader className="space-y-4">
                        <div className="flex justify-between items-start">
                            ...
                            <div className="flex gap-2">
                                <Button variant="outline" size="icon" className="rounded-xl border-white/5 bg-white/5 hover:bg-rose-500/20 hover:text-rose-400 group" onClick={resetPermissions}>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="bg-[#020617] p-3 rounded-2xl border border-white/5">
                                <p className="text-[8px] font-black text-slate-500 uppercase">Coverage</p>
                                <p className="text-sm font-bold text-white">{Math.round((readCount / SYSTEM_ROUTES.length) * 100)}%</p>
                            </div>
                            <div className="bg-[#020617] p-3 rounded-2xl border border-white/5">
                                <p className="text-[8px] font-black text-slate-500 uppercase">Write Slices</p>
                                <p className="text-sm font-bold text-emerald-400">{writeCount}</p>
                            </div>
                            <div className="bg-[#020617] p-3 rounded-2xl border border-white/5">
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-thin scrollbar-thumb-white/10">
                    ...
                    <section className="space-y-4">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Inherited Authority</Label>
                        </div>
                        <CompactSelect
                            label="Role"
                            Icon={Shield}
                            baseStyles="bg-[#0d1117] border-white/5 rounded-2xl h-14 px-6 focus-within:border-blue-500/50 transition-all"
                            props={{ options: ROLES, getOptionLabel: (r) => r.name, getOptionValue: (r) => r.name }}
                            field={{
                                state: { value: activeStaff?.role },
                                handleChange: (val) => {
                                    setStaff(prev => prev.map(s => s.id === selectedId ? { ...s, role: val } : s));
                    ...
                    <section className="space-y-6">
                        <div className="flex flex-col gap-4">
                            <div className="flex justify-between items-end px-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em]">Route Overrides</h3>
                                </div>
                        ...
                        <div className="rounded-[2rem] border border-white/5 bg-[#0d1117]/30 divide-y divide-white/5 overflow-hidden">
                            {filteredRoutes.length > 0 ? filteredRoutes.map((route) => {
                                ...
                                return (
                                    <div key={route.id} className="flex items-center justify-between p-4 px-6 hover:bg-white/[0.02] transition-colors group/row">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[8px] font-mono text-slate-600 mt-0.5 uppercase tracking-tighter">Path: /sys/api/{route.id}</span>
                                        </div>

                                        <div className="flex items-center gap-10">
                                            <Checkbox
                                                checked={perm === "read" || perm === "write"}
                                                onCheckedChange={() => handlePermissionToggle(route.id, "read")}
                                            ...
                                            <Checkbox
                                                checked={perm === "write"}
                                                onCheckedChange={() => handlePermissionToggle(route.id, "write")}
                                ...
                                <div className="p-10 text-center space-y-2">
                                    ...
                                    <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">No matching routes found</p>
                                </div>
                            )}
                        </div>
                    </section>
                    ...
                    <section className="bg-blue-500/5 rounded-[2rem] p-6 border border-blue-500/10 space-y-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h4 className="text-[9px] font-black text-blue-400 uppercase tracking-[0.2em]">Change Log</h4>
                        </div>
