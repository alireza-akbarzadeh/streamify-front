...
export function UserManagementTable() {
    ...
    const table = useReactTable({
        ...
        filterFns: {
            multiSelect: (row, columnId, filterValue) => {
                if (!filterValue || filterValue === "" || (Array.isArray(filterValue) && filterValue.length === 0)) return true;
    ...
    if (error) return (
        <div className="p-16 text-center border-2 border-dashed rounded-[2rem]">
            ...
            <h3 className="font-bold text-lg italic uppercase tracking-tight">Sync Error</h3>
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto">
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48 rounded-xl p-1 shadow-2xl border-border/40">
                                <DropdownMenuItem onClick={() => applySegment('all')} className="text-[10px] font-bold uppercase py-2">All Users</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => applySegment('new')} className="text-[10px] font-bold uppercase py-2 text-amber-600">New Signups</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => applySegment('power')} className="text-[10px] font-bold uppercase py-2 text-primary">Power Users</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => applySegment('risk')} className="text-[10px] font-bold uppercase py-2 text-destructive">At Risk</DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
