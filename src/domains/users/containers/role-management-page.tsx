	...
	const columns = React.useMemo(() => [
		...
		},
		{
			...
			cell: ({ row }: { row: Row<StaffMember> }) => {
				...
				return (
					<span className={cn("px-2.5 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border border-current/20 whitespace-nowrap", role?.color, role?.bg)}>
						{row.original.role}
					</span>
				);
			},
				...
				<div className="flex flex-col w-full relative">
					...
					<div className="p-4 md:p-8 border-b border-white/5 space-y-6 md:space-y-8 bg-gradient-to-b from-blue-500/[0.02] to-transparent">
						<div className="flex justify-between items-start">
							<div className={cn("transition-opacity", isMobileSearchOpen ? "opacity-0" : "opacity-100")}>
							...
							<div className="flex items-center gap-2 md:gap-3">
								...
								<Button variant="outline" className="hidden sm:flex rounded-xl border-white/5 bg-white/5 h-10 md:h-11 px-3 md:px-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest" onClick={handleExport}>
							...
							{Object.keys(rowSelection).length > 0 && (
								<AlertDialog>
									<AlertDialogTrigger asChild>
										<Button variant="destructive" className="h-10 rounded-xl px-4 text-[10px] font-black uppercase animate-in slide-in-from-bottom-2">
									...
									<AlertDialogContent className="bg-[#0d1117] border-white/10 text-white">
										<AlertDialogHeader>
											...
											<AlertDialogDescription className="text-slate-400">
												Immediately revoke access and wipe records from the manifest.
											</AlertDialogDescription>
										</AlertDialogHeader>
										<AlertDialogFooter>
											<AlertDialogCancel className="bg-white/5 border-none">Cancel</AlertDialogCancel>
											<AlertDialogAction onClick={handleBulkDelete} className="bg-rose-600 hover:bg-rose-500">Confirm</AlertDialogAction>
										</AlertDialogFooter>
									</AlertDialogContent>
								</AlertDialog>
							)}
